        ...
        if (row.child.isShown()) {
            ...
            try {
                ...
                const childContent = log.data.map(record => {
                    ...
                    const clockIn = record.clock_in_am instanceof Date ? record.clock_in_am : record.clock_in_am;
                    const breakOut = record.clock_out_am instanceof Date ? record.clock_out_am : record.clock_out_am;
                    const breakIn = record.clock_in_pm instanceof Date ? record.clock_in_pm : record.clock_in_pm;
                    const clockOut = record.clock_out_pm instanceof Date ? record.clock_out_pm : record.clock_out_pm;

                    const consumedHours = clockIn && clockOut
                        ? `${Math.floor((clockOut - clockIn) / 3600000)}h ${Math.floor(((clockOut - clockIn) % 3600000) / 60000)}m`
                        : 'In Progress...';

                    // Check if the origin is biometrics to determine whether to show clock-in and clock-out images
                    const showCapturedImages = record.origin !== 'biometrics'; // assuming 'origin' is the field determining the source
                        ...
                        <div class="mb-3">
                            ...
                            <div class="row">
                                <div class="col-12 col-md-8">
                                    ...
                                    <ul class="my-3">
                                        <li>Employee No: <strong><u>${record.employee_no ?? 'N/A'}</u></strong></li>
                                        <li>Employee Name: <strong><u>${record.information?.personal?.firstname ?? 'N/A'} ${record.information?.personal?.lastname ?? 'N/A'}</u></strong></li>
