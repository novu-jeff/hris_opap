            ...
            <tbody>
                @forelse($salary as $record)
                    <tr data-id="{{$record->id}}">
                        ...
                        <td>
                            @php
                                $displayDate = !empty($record->period) && str_contains($record->period, ' to ')
                                    ? collect(explode(' to ', $record->period))->map(function ($date) {
                                        return \Carbon\Carbon::parse(trim($date))->format('F d, Y');
                                    })->implode(' - ')
                                    : \Carbon\Carbon::parse($record->payroll_date)->format('F d, Y');
                            @endphp
                            {{ \Carbon\Carbon::parse($record->payroll_date)->format('F d, Y') }}
                        </td>
                        <td>{{$record->status}}</td>
