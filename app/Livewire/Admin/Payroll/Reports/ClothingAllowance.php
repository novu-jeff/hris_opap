...
class ClothingAllowance extends Component
{
    ...
    public function deletePayroll($payroll_id) {
        ...
        $payroll = ClothingAllowancePayroll::find($payroll_id);
    ...
    public function render()
    {
        ...
        $records = ClothingAllowancePayroll::when($this->status, fn($q) => $q->where('status', $this->status))
            ->where('employment_type', $employment_type_id)
            ->paginate($this->entries);

        return view('livewire.admin.payroll.reports.clothing-allowance', [
            'salary' => $records
        ]);
    }
}
