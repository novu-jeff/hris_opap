...
class PayrollExportOt implements FromCollection, WithEvents
{
    protected $payroll;
    ...
    public function __construct(OTPayroll $payroll, $filterSalaryMethod = '', $searchName = '', $supervisingOfficer = null, $chiefadministrativeOfficer = null)
    {
                ...
                foreach ($employees as $item) {
                    ...
                    */
                    ...
                    $rows->push([
                        ...
                        $positionWithSalaryGrade,            // G
                        '', '', '', '',                      // H-K
                    
                        $item->basic_salary ?? 0,            // L
                    
                        $this->payroll->period,              // M
                        '',                                  // N
                    
                        formatTime($item->duration, 2),      // O
                        '', '',                              // P-Q
    ...
    if (
        ...
        $sheet->getStyle("B{$row}:T{$row}")
            ...
            ->setVertical(Alignment::VERTICAL_CENTER);
                    ...
                    // BORDERS
                    $sheet->getStyle("A7:T{$highestRow}")
                        ->getBorders()
                        ->getAllBorders()
                        ->setBorderStyle(Border::BORDER_THIN);
                    
                        $sheet->getStyle("A8:T{$highestRow}")
                        ->getAlignment()
                        ->setVertical(Alignment::VERTICAL_CENTER);  
