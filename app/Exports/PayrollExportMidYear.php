        ...
        foreach ($groupedByDepartment as $department => $departmentEmployees) {
            ...
            foreach ($employees as $item) {
                ...
                $rows->push([
                    ...
                    '', '', '', $item->date_hired, '',          // I-M filler
                
                    '', $item->bonus ?? 0,              // N
                    '',                      // O-P filler
                
                                // Q
                    $item->percentage ?? 0,'', $item->net_amount ?? 0, '',                  // R-T filler
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                */
                ...
                $sheet->setCellValue(
                    'A2',
                    'MID YEAR BONUS ' . strtoupper(Carbon::parse($this->payroll->payroll_date)->format('Y'))
                );
...
$sheet->getStyle("A{$footer}:V" . ($lower + 5))
    ...
    ->setVertical(Alignment::VERTICAL_CENTER);
                ...
                $moneyColumns = [
                    'N','O','P','R','S','T','U','V','W','X','Y','Z','AA','AB','AC'
                ];
